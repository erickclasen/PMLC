...
def diffevol():
        ...
        crossp - For each position, we decide (with some probability defined by crossp) if that number will be replaced or not 
        ...
        for m in range(0,5000):
                ...
                for i in range(0,pop_size):
                        ...
                        ''' This one is a test of a weighted value and a bias, of which many can be equal to 5 '''

                        # Create a trial version to test out.
                        trial = np.where(cross_points, mutant, pop[i]) 
                        # Trial prediction using the mutation.  
                        y2 = trial[0] * x + trial[1] ** w  + trial[2] ** wa  + trial[3] ** wb  + trial[4] ** wc  + trial[5] ** wd   

                        # Predict with the i-th population value.
                        y1 = pop[i][0] * x + pop[i][1] ** w + pop[i][2] ** wa + pop[i][3] ** wb + pop[i][4] ** wc + pop[i][5] * wd  
